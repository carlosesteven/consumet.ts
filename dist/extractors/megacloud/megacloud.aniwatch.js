...
exports.getSourcesAniwatch = getSourcesAniwatch;
const axios_1 = __importDefault(require("axios"));
const methods_1 = require("./methods");
async function getSourcesAniwatch(embed_url, site) {
    try {
        const response = await axios_1.default.get('https://raw.githubusercontent.com/yogesh-hacker/MegacloudKeys/refs/heads/main/keys.json');
        ...
        const clientKey = await (0, methods_1.getMegaCloudClientKey)(sourceId);
        if (!clientKey) {
        ...
        const { data: rawSourceData } = await axios_1.default.get(megacloudUrl, { timeout: 5000 });
        ...
        if (!rawSourceData?.encrypted) {
            ...
            const decrypted = (0, methods_1.decryptSrc2)(encrypted, clientKey, megacloudKey);
            try {
        ...
        return {
            ...
            sources: Array.isArray(decryptedSources)
                ? decryptedSources.map((s) => ({
                    file: s.file,
                    type: s.type,
                }))
                : [],
            tracks: Array.isArray(rawSourceData.tracks)
                ? rawSourceData.tracks.map((track) => ({
                    file: track.file,
                    label: track.label,
                    kind: track.kind,
...
//# sourceMappingURL=megacloud.aniwatch.js.map